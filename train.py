...
from models import cls_model, seg_model, local_cls_model, local_seg_model
...
def train(train_dataloader, model, opt, epoch, args, writer):
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        writer.add_scalar('train_loss', loss.detach().item(), step+i)       #Added detach
    # print("Epoch ", epoch, " loss: ", epoch_loss.detach().item())
    return epoch_loss
...
def test(test_dataloader, model, epoch, args, writer):
    ...
    if (args.task == "cls"):
        ...
        for batch in test_dataloader:
            ...
            with torch.no_grad():
                ...
                # print("shape of predicted labels: ", pred_labels)
                # print("\nshape of gt labels: ", labels.data)
    ...
    # Evaluation in Segmentation Task
        ...
        for batch in test_dataloader:
            ...
            with torch.no_grad():     
                pred_labels = model(point_clouds)
                # print("in evaluation, pred_labels: ", pred_labels.shape)
...
def main(args):
    ...
    if args.task == "cls":
        ...
        model = seg_model()
        model.to(dev)
    elif args.task == "cls_loc":
        model = local_cls_model(k=args.k)
        model.to(dev)
        args.task = "cls"
    else:
        model = local_seg_model(k=args.k)
        model.to(dev)
        args.task = "seg"
...
def create_parser():
    ...
    parser.add_argument('--k', type=int, default=8, help='k nearest neighbour')
